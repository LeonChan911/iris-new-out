...
export default async function(projectName: string) {
  ...
  // 5. 整理问询结果
  const formatRes = await formatAnswers(queryRes.result.answers,chooseTempRes.result);
  // 6. 整理模板
...
async function downloadTemplate(downloadUrl: string) {
  ...
  templateTempPath = await download(downloadUrl, process.cwd());
...
// 整理问询结果
async function formatAnswers(ans: { [key: string]: string }, templateName:string) {
  console.log('ans', ans);
  ...
  const reFormatItems: Record<string, string> = keys.reduce((all,s, index) => {
    ...
    return { ...all, [`${templateName}`]: values[index] };
  }, {});
